	...
	// 입력 및 정답 확인 함수
		...
		// 선택버튼(.btn) 이벤트 리스너 
    $('.btn').click(function(){   // 자바스크립트에서는 각각의 버튼이 배열이기 때문에 반복문을 통해 일일이 이벤트 리스터를 지정했지만
                                  // 제이쿼리의 click() 메서드로 각 버튼 클래스에 이벤트를 간단하게 지정할 수 있게 되었다.
			var $answer = $(this).text();   // $(this)는 현재 클릭한 버튼 객체
